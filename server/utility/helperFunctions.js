...
//       .map(
//         (item) => item.productId || (item.product && item.product.toString())
//       ) // Handle both formats
...
//     const cachedPrices = await redisClient.sendCommand([
//       "MGET",
//       ...productIds.map((id) => `product:${id}:price`),
//     ]);
...
//     items.forEach((item) => {
//       const productPrice =
//         pricesMap[item.productId || (item.product && item.product.toString())];
//       if (productPrice) {
...
const calculateTotalPrice = async (items) => {
  try {
    ...
    const products = await Product.find({ _id: { $in: productIds } }, "price");
