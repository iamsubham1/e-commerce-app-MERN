...
const searchedProduct = async (req, res) => {
  try {
    ...
    if (!keyword || keyword.trim() === "") {
      return res
        .status(400)
        .json({ error: "Keyword parameter is required and cannot be empty" });
    }
...
const getProductsByBrand = async (req, res) => {
  try {
    ...
    if (!products || products.length === 0) {
      return res
        .status(404)
        .json({ error: "No products found for the given brand" });
    }
...
const getProductsByCategory = async (req, res) => {
  try {
    ...
    //   const filteredProducts = parsedCachedProducts.filter((product) =>
