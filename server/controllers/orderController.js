...
const addtocart = async (req, res) => {
  try {
    ...
    const response = await shoppingCart.populate(
      "items.product",
      "name reviews pictures price"
    );
...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    if (!["COD", "ONLINE"].includes(paymentMode)) {
      ...
      return res
        .status(400)
        .json({ error: "Invalid payment mode. Must be COD or ONLINE." });
    }
    ...
    try {
      ...
      const response = await fetch(
        "https://e-commerce-app-mern-bmty.onrender.com/api/order/clearcart",
        {
          method: "DELETE",
          headers: {
...
const newPayment = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      redirectUrl: `https://e-commerce-app-mern-bmty.onrender.com/api/order/status/${merchantTransactionId}?data=${encodeURIComponent(
    ...
    console.log("Payment API Request Options:", options);
...
const statusCheck = async (req, res) => {
  try {
    console.log("Status Check Request Params:", req.params);
    console.log("Status Check Request Query:", req.query);
    ...
    if (!data || !userId || !token) {
      console.log("Missing Query Params: data, userId, or token");
    ...
    console.log("X-Verify for Status Check:", xVerify);
    ...
    console.log("Status Check API Request Options:", options);

    const response = await axios.request(options);
    const responseData = response.data;

    console.log("Status Check API Response:", responseData);
    ...
    if (responseData.success) {
      ...
      const createOrderResponse = await fetch(
        "https://e-commerce-app-mern-bmty.onrender.com/api/order/placeorder",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
